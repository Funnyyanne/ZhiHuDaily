...
class ThemeListAdapter(var themes: MutableList<ThemeListGson.OthersBean>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, p1: Int): BaseViewHolder {
        return when (p1) {
            TAG_USERINFOR -> UserInfoViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.navi_item_userinfo, parent, false))
            TAG_MAIN -> MainViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.navi_item_main, parent, false))
            else -> SimpleViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.navi_item, parent, false))
        }
    }

    override fun onBindViewHolder(holder: BaseViewHolder, p1: Int) {
        holder.bindView(p1)
        holder.onSelected(selectedPosition == p1)
    }
    ...
    inner class MainViewHolder(rootView: View): BaseViewHolder(rootView) {
        ...
        override fun bindView(position: Int) {
            ...
            itemView.setOnClickListener { v ->
                drawerItemClick.invoke(TAG_MAIN, 0, "首页")
